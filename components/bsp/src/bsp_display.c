...
esp_err_t bsp_display_init(void) {
    ...
    esp_lcd_panel_mirror(s_panel, true, false);                  // Normal landscape orientation
    esp_lcd_panel_set_gap(s_panel, 0, 0);
    esp_lcd_panel_disp_on_off(s_panel, true);                    // 0x29 DISPON

    backlight_init();
    ESP_LOGI(TAG, "显示就绪 (Landscape %dx%d)", BSP_LCD_H, BSP_LCD_W);
    return ESP_OK;
}

esp_err_t bsp_display_set_rotation(bool swap_xy, bool mirror_x, bool mirror_y) {
    if (!s_panel) return ESP_ERR_INVALID_STATE;
    esp_err_t ret = esp_lcd_panel_swap_xy(s_panel, swap_xy);
